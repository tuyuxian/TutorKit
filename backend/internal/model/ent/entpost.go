...
type EntPostEdges struct {
	...
	// ShareWith holds the value of the shareWith edge.
	ShareWith []*EntUser `json:"shareWith,omitempty"`
...
// ShareWithOrErr returns the ShareWith value or an error if the edge
// was not loaded in eager-loading.
func (e EntPostEdges) ShareWithOrErr() ([]*EntUser, error) {
	if e.loadedTypes[1] {
		return e.ShareWith, nil
	}
	return nil, &NotLoadedError{edge: "shareWith"}
}
...
// QueryShareWith queries the "shareWith" edge of the EntPost entity.
func (ep *EntPost) QueryShareWith() *EntUserQuery {
	return (&EntPostClient{config: ep.config}).QueryShareWith(ep)
}
