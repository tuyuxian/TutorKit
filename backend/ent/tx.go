...
func (tx *Tx) init() {
	tx.EntAttendance = NewEntAttendanceClient(tx.config)
	tx.EntComment = NewEntCommentClient(tx.config)
	tx.EntCourse = NewEntCourseClient(tx.config)
	tx.EntPost = NewEntPostClient(tx.config)
	tx.EntTodo = NewEntTodoClient(tx.config)
...
// applies a query, for example: EntAttendance.QueryXXX(), the query will be executed
